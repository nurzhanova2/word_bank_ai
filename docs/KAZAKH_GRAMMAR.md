...
полный текст
  ...
  → при низкой полноте длинного текста: дополнительный audit по абзацам
...
позволяет длинному списку OOV-слов вытеснить синтаксический анализ. Если длинный текст
возвращает подозрительно мало ошибок относительно числа предложений, runtime повторно
проверяет каждый абзац и переводит локальные UTF-16 offsets обратно в координаты полного текста.
Ответы используют schema v2:
