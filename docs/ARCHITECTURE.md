...
## Grammar Engine 0.2.0

`packages/local-runtime/src/grammar/` содержит единый контракт грамматических
движков, детектор языка, клиент LanguageTool, LLM fallback и orchestration service.
...
проверяет локальный LanguageTool; казахские и недоступные локальному серверу
сегменты переходят к Qwen. Пересекающиеся исправления фильтруются, а остальные
...
официальные архивы во время сборки. Бинарники исключены из Git, но включаются в
