...
Каталог промтов имеет собственную версию в `actions/prompts/version.ts`, поэтому
изменение инструкций можно отслеживать независимо от версии EXE. Для перевода
`actions/glossary.ts` добавляет только словарь выбранного целевого языка. Исходный
текст XML-кодируется модулем `actions/source-envelope.ts`: теги или инструкции из
документа остаются данными и не могут закрыть пользовательскую оболочку `source`.
Eval-контракты поддерживают как обязательные `includes`, так и запрещённые
`excludes` — например, вводные фразы модели и лишние заголовки.

## Grammar Engine 0.3.0

`packages/local-runtime/src/grammar/` содержит единый контракт грамматических
движков, детектор языка, клиент LanguageTool, Hunspell-движок, казахские правила,
LLM fallback и orchestration service.
Каждая ошибка содержит точное смещение, исходный фрагмент, категорию, объяснение,
варианты замены, confidence, rule ID и источник. Русские и английские сегменты
проверяет локальный LanguageTool. Казахские сегменты проверяются локально словарём
Hunspell и первой версией объяснимых правил: повтор слов, пробел перед пунктуацией,
смешение латиницы с кириллицей и частые ошибки казахской раскладки. При отказе
совместимого движка используется следующий безопасный fallback. Пересекающиеся исправления фильтруются, а остальные
применяются справа налево без сдвига координат.

Desktop Host запускает LanguageTool через встроенный Eclipse Adoptium JRE. Скрипт
`packages/desktop-host/scripts/prepare-grammar.ps1` воспроизводимо скачивает
официальные архивы и открытый словарь `myspell-kk` во время сборки. Бинарники и
