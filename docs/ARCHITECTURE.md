...
- грамматические исправления применяются отдельно либо общей кнопкой после предпросмотра;
...
смешение латиницы с кириллицей и частые ошибки казахской раскладки.
Hunspell используется только как детектор неизвестных слов: его edit-distance варианты записываются
в `suggestions` с `autoApply: false` и показываются пользователю для справки. Поле `replacements`
остаётся пустым, поэтому словарь не может заменить корректную агглютинативную форму. Автоматическое
исправление возможно только от детерминированного правила или после независимого подтверждения Qwen.
При отказе совместимого движка локальный результат сохраняется. После локальной проверки настроенная Qwen
выполняет контекстный review исходного текста: находит пропущенные согласования и контекстные
опечатки вроде `оплота → оплата`. Модель отвечает по строгому JSON Schema версии 1 отдельными
позиционными исправлениями. Runtime проверяет типы, UTF-16 диапазоны, точное совпадение исходных
фрагментов, пересечения и защищённые термины. Реквизиты перед review заменяются равнодлинной маской,
которая скрывает значения без сдвига UTF-16 координат; невалидный ответ полностью отбрасывается, не затрагивая
результаты локальных движков. Проверенные находки объединяются с локальными исправлениями.
Пересекающиеся исправления фильтруются, а остальные
применяются справа налево без сдвига координат.

В панели Word каждую исправляемую находку можно применить отдельно. Кнопка «Исправить всё»
применяет единый проверенный `correctedText`; неисправляемые автоматически замечания остаются
помеченными для ручной проверки.
