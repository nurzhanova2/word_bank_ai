...
опечатки вроде `оплота → оплата`. Ответ модели разбивается на отдельные позиционные изменения,
объединяется с локальными находками и не может заменить защищённые банковские термины.
