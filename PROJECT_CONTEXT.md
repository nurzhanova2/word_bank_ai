...
5. Если provider не mock, после локального этапа Qwen проверяет весь исходный текст. Для казахского он получает также все Hunspell candidates и выполняет независимо full-context analysis и candidate validation. Реквизиты заменяются равнодлинной маской `¤`, чтобы сохранить offsets. Казахский ответ соответствует JSON Schema v2 (`errors` + обязательный `hunspell_validation`), русский/английский generic review сохраняет schema v1.
...
7. Для каждого казахского кандидата требуется `ACCEPT / REJECT / UNCERTAIN`. REJECT удаляет ложное предупреждение, UNCERTAIN оставляет исходник для ручной проверки. Пороги auto/review по умолчанию 0.90/0.70 и настраиваются через environment. Находки объединяются по диапазону, предпочтение отдаётся применимой и затем более уверенной. Автоматические изменения применяются справа налево.
